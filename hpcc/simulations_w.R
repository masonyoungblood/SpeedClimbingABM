...
SpeedClimbingABM_slurm <- function(innov_prob, innov_x_times, innov_x_pop, innov_x_year, learn_prob, learn_x_times, learn_x_pop, learn_x_year, n_top, constraint_a, constraint_b, improve_rate_m, improve_rate_sd, improve_min){
...
for(i in 1:rounds){
  if(i == 1){
    ...
    priors <- data.frame(innov_prob = rbeta(n_sim, 1, 2),
                         ...
                         improve_min = truncnorm::rtruncnorm(n_xim, 0, 1, 0.4114153, 0.025))
  } else{
