...
SpeedClimbingABM_slurm <- function(innov_prob, innov_x_times, innov_x_pop, innov_x_year, learn_prob, learn_x_times, learn_x_pop, learn_x_year, n_top, constraint_b, improve_rate_m, improve_min){
  temp <- SpeedClimbingABM(n = n, years = years, pop_data = pop_data, grid = grid, n_holds = 20,
                           ...
                           improve_rate_m = improve_rate_m, improve_min = 0.3527184, sum_stats = FALSE, plot = FALSE)
...
for(i in 1:rounds){
  if(i == 1){
    ...
    rm(list = c("innov_prob_post", "innov_x_times_post", "innov_x_pop_post", "innov_x_year_post",
                ...
                "n_top_post", "constraint_b_post", "improve_rate_m_post"))
  }
