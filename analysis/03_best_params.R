...
#process and save observed data in identical format
obs_data <- lapply(2:length(years), function(x){
  (c(sort(data$time[which(data$year == years[x])]), rep(sim_mean, max(n) - n[x])) - sim_mean)/sim_sd
})
obs_data <- list(obs_data)
jsonlite::write_json(obs_data, "analysis/data_and_output/03_best_params/obs_data.json")
...
train_data <- list(formatted_priors[1:99000], formatted_simulations[1:99000])
test_data <- list(formatted_priors[99001:100000], formatted_simulations[99001:100000])
jsonlite::write_json(train_data, "analysis/data_and_output/03_best_params/train_data.json")
jsonlite::write_json(test_data, "analysis/data_and_output/03_best_params/test_data.json")

#combine prior and simulations into single object and save
priors_and_simulations <- list(priors, simulations)
save(priors_and_simulations, file = "analysis/data_and_output/03_best_params/priors_and_simulations.RData")
