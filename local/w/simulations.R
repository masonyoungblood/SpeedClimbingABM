...
# 
# #run simulations with only innovation and improvement varied
# simulations_a <- parallel::mclapply(1:n_sim, function(x){
...
#                    max_dist = 1.645,
#                    improve_min = 0.3527184, sum_stats = FALSE, plot = FALSE)
# }, mc.cores = 7)
# 
# #run simulations with innovation, improvement, and learning varied with fixed n_top
# simulations_b <- parallel::mclapply(1:n_sim, function(x){
#   SpeedClimbingABM(n = n, years = years, pop_data = pop_data, grid = grid, n_holds = 20,
#                    beta_true_prob = 1,
#                    innov_prob = priors$innov_prob[x],
...
# simulations_c <- parallel::mclapply(1:n_sim, function(x){
#   SpeedClimbingABM(n = n, years = years, pop_data = pop_data, grid = grid, n_holds = 20,
#                    beta_true_prob = 1,
#                    innov_prob = priors$innov_prob[x],
#                    learn_prob = priors$learn_prob[x],
...
# priors_and_simulations <- list(priors, simulations_a, simulations_b, simulations_c)
# save(priors_and_simulations, file = "local/w/priors_and_simulations.RData")

#load data
load("local/w/priors_and_simulations.RData")

#load into separate objects
priors <- priors_and_simulations[[1]]
simulations <- priors_and_simulations[[4]]

#set number of simulations
n_sim <- nrow(priors)

# #expand into summary statistics object for prediction
# sum_stats <- do.call(rbind, parallel::mclapply(1:n_sim, function(y){unlist(simulations[[y]][-1])}, mc.cores = 7))
# 
# #save it
# save(sum_stats, file = "local/w/sum_stats.RData")
...
#                            trControl = trainControl(number = n_valid, verboseIter = FALSE),
#                            tuneGrid = xgb_grid)
# 
#   #print update
...
#                            trControl = trainControl(number = n_valid, search = "random", verboseIter = FALSE),
#                            tuneLength = n_tune)
# 
#   #print update
...
#   output <- list(glm = glm_caret_model$results,
#                  rf = rf_caret_model$results,
#                  xgb = xgb_caret_model$results,
#                  dnn = dnn_caret_model$results)
# 
#   #remove all objects
#   rm(list = c("glm_caret_model", "rf_caret_model", "xgb_caret_model", "dnn_caret_model"))
# 
#   #print update
...
# 
# #save abc models
# save(tuning, file = "local/w/tuning.RData")

#load data
load("local/w/tuning.RData")
...
for(x in 1:4){
  ...
  #save model
  save(model, file = paste0("local/w/models/", colnames(priors)[x], "_model.RData"))
