...
#load models
load("local/models/innov_prob_model.RData")
innov_prob_model <- model
rm(model)
load("local/models/learn_prob_model.RData")
learn_prob_model <- model
rm(model)
load("local/models/n_top_model.RData")
n_top_model <- model
rm(model)
load("local/models/improve_rate_m_model.RData")
...
general_sims <- do.call(rbind, parallel::mclapply(1:n_iter, function(x){
  observed <- SpeedClimbingABM(n = n, years = years, pop_data = pop_data, grid = grid, n_holds = 20,
                               beta_true_prob = 1, max_dist = 1.645, improve_min = 0.3427374, 
                               innov_prob = known[x],
...
save(predictions, file = "local/predictions.RData")
