...
# save(priors_and_simulations, file = "local/priors_and_simulations.RData")

#load data
load("local/priors_and_simulations.RData")
...
# 
# #save it
# save(sum_stats, file = "local/sum_stats.RData")
...
tuning <- lapply(1:5, function(x){
  ...
  output <- list(xgb = xgb_caret_model$results,
                 dnn = dnn_caret_model$results)

  #remove all objects
  # rm(list = c("glm_caret_model", "rf_caret_model", "xgb_caret_model", "dnn_caret_model"))
  rm(list = c("xgb_caret_model", "dnn_caret_model"))
...
#save abc models
save(tuning, file = "local/tuning.RData")

#load data
load("local/tuning.RData")
...
for(x in 1:4){
  ...
  #save model
  save(model, file = paste0("local/models/", colnames(priors)[x], "_model.RData"))
